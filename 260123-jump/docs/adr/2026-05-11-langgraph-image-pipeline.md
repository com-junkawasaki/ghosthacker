...
### Phase C 検証 (2026-05-11)

```
SVG pages:                46/46 (44 + 2 spread halves)
PNG pages:                46/46 @ 150 dpi (page-01: 10.6 MB, page-39 spread: 19.7 MB)
PDF size:                 449 MB (uncompressed PNG embedded)
Vertical bubbles (p01):   40 columns
SFX glyphs (p35):         5 rendered
Font import per SVG:      1 (`Noto Serif JP`)
panelOverflow coverage:   279/279 (100%)
```

入稿 workflow: `arc0-1-origin.pdf` → 印刷所 (B5 / A5 製本) もしくは electron-pdf preview。
