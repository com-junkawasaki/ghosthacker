...
(defn- kw-name
  ...
  **数字始まりは `_` を付けて逃がす。** 実測（2026-08-13）: cljs の reader は
  裸の `:9bad` は受理するが、**名前空間付きの `:env/0KVh…` は
  `Invalid keyword` で落ちる**。ここで作るのは常に名前空間付きなので、
  `:episode/260123-cschool-defamation` は read できない。
