...
#[macro_export]
macro_rules! handle_sqlx_error {
    ($result:expr) => {
        $result.map_err($crate::service::error::sqlx_error_to_status)
    };
}

#[macro_export]
macro_rules! handle_uuid_error {
    ($result:expr) => {
        $result.map_err($crate::service::error::uuid_error_to_status)
    };
}
