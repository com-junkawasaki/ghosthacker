...
import type { PanelSelectionState } from '@/types/manga';
import { matchPanelSelection } from '@/types/manga';
...
export function PanelSettingsSection({ 
  ...
  const selectionState: PanelSelectionState = useMemo(() => {
    if (!selectedPanel) {
      return { type: 'none' };
    }
    return {
      ...
      panel: {
        ...
        hideBorder: selectedPanel.hideBorder,
        ignoreNeighborPanels: selectedPanel.ignoreNeighborPanels,
      },
    };
  }, [selectedPanel]);
  ...
  // Update local state when selectedPanel changes
  useEffect(() => {
    if (selectedPanel) {
      setOrder(selectedPanel.order ?? 1);
      setHideBorder(selectedPanel.hideBorder ?? false);
      setIgnoreNeighborPanels(selectedPanel.ignoreNeighborPanels ?? false);
    }
  }, [selectedPanel]);
  ...
  return matchPanelSelection(selectionState, {
    none: () => (
