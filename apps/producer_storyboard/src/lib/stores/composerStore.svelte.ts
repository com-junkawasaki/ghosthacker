...
function createComposerStore() {
	...
	return {
		...
		moveClip(clipId: string, newStartTime: number, newTrackId?: string) {
			let movedClip: Clip | undefined = undefined;
			
			// Remove clip from current track
			state.tracks = state.tracks.map(track => {
				const clip = track.clips.find(c => c.id === clipId);
				if (clip) {
					const updatedClip: Clip = { ...clip, startTime: Math.max(0, newStartTime) };
					if (newTrackId) {
						updatedClip.trackId = newTrackId;
					}
					movedClip = updatedClip;
					return {
						...track,
						clips: track.clips.filter(c => c.id !== clipId),
					};
			...
			if (movedClip !== undefined) {
				const clipToAdd: Clip = movedClip;
