/**
 * Backgrounds API Endpoint
 * Proxies background image requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListBackgrounds', { projectId }, headers);
		
		if (!response.ok) {
			console.error('[Backgrounds API] Error from gRPC');
			return json({ items: [] });
		}
		
		const data = await response.json();
		return json({ items: data.backgrounds || [] });
	} catch (error) {
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	try {
		// For now, create the record without file upload
		// File upload would need to be handled by a separate service
		...
		if (!createResponse.ok) {
			const errorData = await createResponse.json().catch(() => ({ error: 'Failed to create background' }));
		...
		console.error('[Backgrounds API] Error:', error);
		return json({ error: 'Failed to create background' }, { status: 500 });
	}
};
