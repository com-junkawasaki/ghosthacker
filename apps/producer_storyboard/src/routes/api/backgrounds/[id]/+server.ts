/**
 * Background Individual API Endpoint
 */
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteBackground', { id }, headers);
		
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({ error: 'Failed to delete background' }));
