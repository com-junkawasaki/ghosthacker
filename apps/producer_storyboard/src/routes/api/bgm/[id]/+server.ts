/**
 * BGM Individual API Endpoint
 * Handles GET, PUT, DELETE for individual BGM tracks
 */
...
export const GET: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('GetBGM', { id }, headers);
		
		if (!response.ok) {
			return json({ error: 'BGM not found' }, { status: 404 });
		}
		...
		return json({ error: 'Failed to get BGM' }, { status: 500 });
	}
};
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteBGM', { id }, headers);
		
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({ error: 'Failed to delete BGM' }));
		...
		console.error('[BGM API] Error:', error);
		return json({ error: 'Failed to delete BGM' }, { status: 500 });
	}
};
