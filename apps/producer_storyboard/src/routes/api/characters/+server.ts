 ...
 * Proxies character-related requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ request, url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListCharacters', { projectId }, headers);
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	const { projectId, name, description, personality, background, defaultHumeVoiceId } = body;
	...
	try {
		const response = await callGrpcService(
			'CreateCharacter',
			{ projectId, name, description, personality, background, defaultHumeVoiceId },
			headers
		);
		const data = await response.json();
		return json(data);
