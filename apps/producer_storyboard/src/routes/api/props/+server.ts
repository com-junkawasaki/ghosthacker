/**
 * Props API Endpoint
 * Proxies prop asset requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListProps', { projectId }, headers);
		
		if (!response.ok) {
			console.error('[Props API] Error from gRPC');
			return json({ items: [] });
		}
		
		const data = await response.json();
		return json({ items: data.props || [] });
	} catch (error) {
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	try {
		...
		if (!createResponse.ok) {
			const errorData = await createResponse.json().catch(() => ({ error: 'Failed to create prop' }));
		...
		console.error('[Props API] Error:', error);
		return json({ error: 'Failed to create prop' }, { status: 500 });
	}
};
