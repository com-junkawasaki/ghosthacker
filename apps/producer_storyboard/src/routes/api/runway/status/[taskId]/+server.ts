/**
 * Runway Video Task Status API Endpoint
 * Proxies status requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({ error: 'Failed to get task status' }));
