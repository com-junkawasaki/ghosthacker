/**
 * Scene API Endpoint (single scene)
 * Proxies scene CRUD requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('GetScene', { id: sceneId }, headers);
...
export const PUT: RequestHandler = async ({ params, request, cookies, locals }) => {
	...
	const { textDescription, durationSeconds, startTimeSeconds, transitionType } = body;
	...
	try {
		const response = await callGrpcService(
			'UpdateScene',
			{ id: sceneId, textDescription, durationSeconds, startTimeSeconds, transitionType },
			headers
		);
		const data = await response.json();
		return json(data);
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteScene', { id: sceneId }, headers);
		const data = await response.json();
		return json(data);
	} catch (error) {
		console.error('[Scene API] Error:', error);
