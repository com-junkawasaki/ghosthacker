 ...
 * Proxies storyboard-related requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ request, url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListStoryboards', { projectId }, headers);
		const data = await response.json();
		return json(data);
	} catch (error) {
		console.error('[Storyboards API] Error:', error);
		return json({ error: 'Failed to fetch storyboards' }, { status: 500 });
	}
};

export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	const body = await request.json();
	const { projectId, title, aspectRatio, resolution, durationSeconds, numVariations } = body;
	...
	try {
		const response = await callGrpcService(
			'CreateStoryboard',
			{ projectId, title, aspectRatio, resolution, durationSeconds, numVariations },
			headers
		);
		const data = await response.json();
		return json(data);
		...
		console.error('[Storyboards API] Error:', error);
		return json({ error: 'Failed to create storyboard' }, { status: 500 });
	}
};
