/**
 * Tag API Endpoint (individual tag operations)
 */
...
export const GET: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('GetTag', { id: tagId }, headers);
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteTag', { id: tagId }, headers);
		const data = await response.json();
		return json(data);
	} catch (error) {
		console.error('[Tag API] Error:', error);
