...
;;   bb edn-datomize.bb gh-dir   <dir>            — 既存 :gh/:dct/:prov/:schema
;;                                                    名前空間を温存しつつ bare キーだけ
;;                                                    :gh/* に昇格する汎用変換
;;
;; ghosthacker 向け改変: schema-path をこの子リポのルート（manifest/ 無し）に変更し、
;; ADR frontmatter 専用の adr-key/transform-adr-generic とは別に、既に JSON-LD 由来の
;; 名前空間付きキー（:gh/* :dct/* :prov/* :schema/*）を使っているこのリポの資源データ
;; 向けに gh-key/transform-gh-generic + gh-dir! を追加した（Phase 2 の
;; namespace-preserving pilot と同型）。
...
;; ghosthacker には manifest/ ディレクトリが無いので、この子リポ自身のルートに置く。
...
(defn merge-schema! [new-attrs]
  (let [existing (load-schema)
    ...
    (spit (schema-path) (str ";; schema.edn — Datomic/Datascript 互換スキーマ定義（自動生成 by edn-datomize.bb）\n"
                              ...
                              ";; :schema/* の名前空間付きキーを使っており、gh-dir モードはこれを温存しつつ\n"
                              ";; bare top-level キーだけ :gh/* に昇格した(namespace-preserving、Phase 2 の\n"
                              ";; net-kotobase/security pilot と同型)。同じ属性名でも入れ子構造を含む場合は\n"
                              ";; blob(:db.type/string 化した pr-str)になるため、ファイルによって同一 ident の\n"
                              ";; 実データ shape が scalar/blob で揺れることがある — 最初に遭遇した型のみ登録。\n\n"
                              (pr-str merged)
                              "\n"))
    merged))
...
;; ---------- ghosthacker resources (namespace-preserving) ----------
;;
;; 実測(2026-07-10): 260123-jump/resources/** ・ 260122-presentation/data/** ・
;; apps/web/PROJECT.edn 等、145 ファイル中 144 件がトップレベル map で、すでに
;; JSON-LD (*.jsonld → *.edn 変換、README記載) 由来の名前空間付きキー
;; (:gh/* :dct/* :prov/* :schema/*) を使っている。bare な top-level キーは
;; :context :id :links :meta :name :nodes :transform :type の8種のみで、
;; いずれもこのリポ自身の JSON-LD context（:gh "https://ghosthacker.gftd.ai/ns/"）
;; の語彙に属するため :gh/* に昇格する（ADR の adr-key と同型、ns だけ "gh"）。
;; 1件（resources/_index.edn）はキーワード名が数字始まり
;; (:env/0KVhLEQygO_kN0RTS44sc) で EDN として不正 = pre-existing corrupt、skip。

(defn gh-key [k]
  (if (namespace k) k (keyword "gh" (name k))))

(defn transform-gh-generic [content]
  [(into {:db/id -1}
         (map (fn [[k v]] [(gh-key k) (attr-value v)]))
         content)])

(defn gh-schema-for [entity]
  (for [[k v] (dissoc entity :db/id)]
    (let [{:keys [type card]} (classify v)]
      {:db/ident k :db/valueType type :db/cardinality card})))

(defn gh-file! [f report]
  (try
    (let [content (slurp-edn f)]
      (cond
        (already-tx-data? content)
        ...
        (not (map? content))
        (do (println "skip (not a top-level map):" (str f))
            (swap! report update :skipped conj (str f)))

        :else
        (let [tx (transform-gh-generic content)]
          (spit f (pr-str tx))
          (swap! report update :attrs into (mapcat gh-schema-for tx))
          (swap! report update :ok conj (str f)))))
    (catch Exception e
      (println "SKIP (parse/transform error):" (str f) "->" (.getMessage e))
      (swap! report update :errors conj [(str f) (.getMessage e)]))))
...
(defn gh-dir! [dir]
  (let [files (->> (io/file root dir) file-seq (filter #(str/ends-with? (str %) ".edn")) sort)
        report (atom {:ok [] :skipped [] :errors [] :attrs []})]
    (doseq [f files] (gh-file! f report))
    (merge-schema! (:attrs @report))
    (println "done." (count files) "files:" (count (:ok @report)) "transformed,"
             (count (:skipped @report)) "skipped," (count (:errors @report)) "errors.")
    (when (seq (:errors @report))
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (println @report))
      "gh-dir"   (gh-dir! a)
      (do (println "usage: bb edn-datomize.bb [wrap-map <path> <ns> | adr-dir <dir> | adr-file <path> | gh-dir <dir>]")
          (System/exit 1)))))
