/**
 * EPUB Document gRPC API Route (by ID)
 * Next.js API route for individual EPUB document operations via gRPC
 */
...
import type {
  GetEPUBDocumentRequest,
  GetEPUBDocumentResponse,
...
export async function GET(
  ...
  try {
    ...
    const req: GetEPUBDocumentRequest = { id: params.id };

    return new Promise<NextResponse>((resolve) => {
      client.getEPUBDocument(req, (err: grpc.ServiceError | null, response?: GetEPUBDocumentResponse) => {
        if (err) {
          console.error('gRPC error:', err);
          resolve(
            NextResponse.json(
              { error: err.message || 'Failed to get EPUB document' },
              { status: 500 }
            )
          );
          return;
        ...
        resolve(NextResponse.json({ epubDocument: document }));
      });
    });
  } catch (error) {
    console.error('Error in GET /api/grpc/epub/[id]:', error);
...
export async function PUT(
  ...
  try {
    ...
    const client = getClient();
    const req: UpdateEPUBDocumentRequest = {
      id: params.id,
      title,
    ...
    return new Promise<NextResponse>((resolve) => {
      client.updateEPUBDocument(req, (err: grpc.ServiceError | null, response?: UpdateEPUBDocumentResponse) => {
        if (err) {
          console.error('gRPC error:', err);
          resolve(
            NextResponse.json(
              { error: err.message || 'Failed to update EPUB document' },
              { status: 500 }
            )
          );
          return;
    ...
    console.error('Error in PUT /api/grpc/epub/[id]:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
