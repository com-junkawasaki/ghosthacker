...
-- This migration creates tables for scenario management with hierarchical structure (scenario -> episode -> part -> scene_plan)
...
BEGIN
    IF NEW.org_id IS DISTINCT FROM OLD.org_id THEN
        ...
        -- Update episodes through scenarios
        UPDATE episodes
        SET org_id = NEW.org_id
        ...
        -- Update parts through episodes
        UPDATE parts
        SET org_id = NEW.org_id
        WHERE episode_id IN (
            SELECT id FROM episodes WHERE scenario_id IN (
                SELECT id FROM scenarios WHERE project_id = NEW.id
            )
        );
        
        -- Update scene_plans through parts
        UPDATE scene_plans
        SET org_id = NEW.org_id
        WHERE part_id IN (
            SELECT id FROM parts WHERE episode_id IN (
                SELECT id FROM episodes WHERE scenario_id IN (
                    SELECT id FROM scenarios WHERE project_id = NEW.id
                )
            )
