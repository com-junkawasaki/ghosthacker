/**
 * Composers API Endpoint
 * Proxies composer-related requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ request, url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListComposers', { projectId }, headers);
		const data = await response.json();
		return json(data);
	} catch (error) {
		console.error('[Composers API] Error:', error);
		return json({ error: 'Failed to fetch composers' }, { status: 500 });
	}
};

export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	const body = await request.json();
	const { projectId, title, durationSeconds } = body;
	...
	try {
		const response = await callGrpcService(
			'CreateComposer',
			{ projectId, title, durationSeconds },
			headers
		);
		const data = await response.json();
		return json(data);
