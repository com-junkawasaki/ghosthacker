/**
 * Dialog Individual API Endpoint
 */
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteDialog', { id }, headers);
		
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({ error: 'Failed to delete dialog' }));
		...
		return json({ error: 'Failed to delete dialog' }, { status: 500 });
	}
};
