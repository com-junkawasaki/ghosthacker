...
export const GET: RequestHandler = async ({ url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListEnvironments', { projectId }, headers);
		
		if (!response.ok) {
			console.error('[Environments API] Error from gRPC');
			return json({ items: [] });
		}
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	try {
		...
		if (!createResponse.ok) {
			const errorData = await createResponse.json().catch(() => ({ error: 'Failed to create environment' }));
