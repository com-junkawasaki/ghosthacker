/**
 * Manga gRPC-Connect API Proxy Endpoint (Catch-all route)
 * Proxies requests to the manga grpc-go backend service
...
export const POST: RequestHandler = async ({ request, cookies, params }) => {
	...
	// Extract service and method from params.path
	// params.path will be "manga.v1.MangaService/ListMangaProjects"
	...
	const servicePath = pathParts.slice(0, -1).join('/');
	
	// Construct the Connect endpoint URL (Connect RPC requires leading slash)
	const connectPath = servicePath ? `/${servicePath}/${serviceMethod}` : `/${serviceMethod}`;
	const targetUrl = `${grpcApiUrl}${connectPath}`;
	
	// Get Clerk session from cookies or headers
	const url = new URL(request.url);
	...
	try {
		...
		if (!response.ok) {
			console.error('[Manga gRPC Proxy] Error response:', {
		...
		console.error('[Manga gRPC Proxy] Error forwarding request:', {
		...
		return json(
			{ 
				error: 'Failed to forward request to manga grpc-go service',
