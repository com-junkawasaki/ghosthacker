/**
 * Pieces API Endpoint
 * Proxies piece asset requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListPieces', { projectId }, headers);
		
		if (!response.ok) {
			console.error('[Pieces API] Error from gRPC');
			return json({ items: [] });
		}
		
		const data = await response.json();
		return json({ items: data.pieces || [] });
	} catch (error) {
		console.error('[Pieces API] Error:', error);
		return json({ items: [] });
	}
};
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	try {
		...
		if (!createResponse.ok) {
			const errorData = await createResponse.json().catch(() => ({ error: 'Failed to create piece' }));
