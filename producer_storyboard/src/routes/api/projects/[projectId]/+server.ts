/**
 * Project API Endpoint (single project)
 * Proxies project CRUD requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('GetProject', { id: projectId }, headers);
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteProject', { id: projectId }, headers);
		const data = await response.json();
		return json(data);
	} catch (error) {
		console.error('[Project API] Error:', error);
