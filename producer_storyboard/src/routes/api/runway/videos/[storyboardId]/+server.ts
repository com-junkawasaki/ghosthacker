/**
 * List Generated Videos API Endpoint
 * Proxies video list requests to grpc-go service
 */
...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({ error: 'Failed to list generated videos' }));
			return json(errorData, { status: response.status });
		}

		const data = await response.json();
		return json({
			videos: (data.videos || []).map((video: Record<string, unknown>) => ({
				id: video.id,
				storyboardId: video.storyboardId,
				variationNumber: video.variationNumber,
		...
		console.error('Failed to list generated videos:', error);
		return json({ error: 'Failed to list generated videos' }, { status: 500 });
	}
};
