/**
 * Generate Scene Image API Endpoint
 * Proxies scene image generation requests to grpc-go service
 */
...
export const POST: RequestHandler = async ({ params, request, cookies, locals }) => {
	...
	const { prompt, model, imageType } = body;
	...
	try {
		const response = await callGrpcService(
			'GenerateSceneImage',
			{ sceneId, prompt, model, imageType },
			headers
		);
		const data = await response.json();
		return json(data);
	} catch (error) {
		console.error('[Generate Scene Image API] Error:', error);
