 ...
 * Proxies sound effects requests to grpc-go service
...
export const GET: RequestHandler = async ({ url, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('ListSoundEffects', { projectId }, headers);
		
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({}));
			console.error('[Sound Effects API] Error from gRPC:', errorData);
			// Return empty list if no sound effects exist yet
			return json({ items: [] });
		}
		
		const data = await response.json();
		return json({ items: data.soundEffects || [] });
	} catch (error) {
...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	...
	try {
		// First create the sound effect record
		const createResponse = await callGrpcService(
			'CreateSoundEffect',
			{
				projectId,
				name: name || 'Generated Sound Effect',
				prompt,
				status: 'generating',
			},
			headers
		...
		if (!createResponse.ok) {
			const errorData = await createResponse.json().catch(() => ({ error: 'Failed to create sound effect' }));
			return json(errorData, { status: createResponse.status });
		}

		const createdEffect = await createResponse.json();

		// Then trigger Suno generation
		const generateResponse = await callGrpcService(
			'GenerateSunoAudio',
			{
				recordId: createdEffect.id,
				recordType: 'sound_effect',
				prompt,
				makeInstrumental: makeInstrumental ?? false,
		...
		if (!generateResponse.ok) {
			console.warn('[Sound Effects API] Suno generation failed:', await generateResponse.text());
			// Still return the created record, generation will be retried
		}

		return json(createdEffect);
	} catch (error) {
		console.error('[Sound Effects API] Error:', error);
		return json({ error: 'Failed to create sound effect' }, { status: 500 });
	}
};
