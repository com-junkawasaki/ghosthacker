/**
 * Sound Effect Individual API Endpoint
 * Handles GET, PUT, DELETE for individual sound effects
 */
...
export const GET: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('GetSoundEffect', { id }, headers);
		
		if (!response.ok) {
			return json({ error: 'Sound effect not found' }, { status: 404 });
		}
		...
		return json({ error: 'Failed to get sound effect' }, { status: 500 });
	}
};
...
export const DELETE: RequestHandler = async ({ params, cookies, locals }) => {
	...
	try {
		const response = await callGrpcService('DeleteSoundEffect', { id }, headers);
		
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({ error: 'Failed to delete sound effect' }));
		...
		console.error('[Sound Effects API] Error:', error);
		return json({ error: 'Failed to delete sound effect' }, { status: 500 });
	}
};
