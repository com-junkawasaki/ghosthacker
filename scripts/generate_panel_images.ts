...
async function main() {
  ...
  const storyboardPath = (args["storyboard"] as string) || path.join(process.cwd(), "251121", "storyboard.jsonld");
  const outDir = (args["out"] as string) || path.join(process.cwd(), "251121", "panel_images");
