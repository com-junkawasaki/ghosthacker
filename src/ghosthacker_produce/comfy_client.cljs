(ns ghosthacker-produce.comfy-client
  ...
  (:require ["fs" :as fs]
            ...
            [ghosthacker-produce.comfy-graph :as g]))
...
(defn render-panel!
  "Panel -> a promise of {:ok? bool :file path :reason ...}.
  ...
  [{:keys [base out-dir panel config timeout-ms poll-ms]
    :or {timeout-ms 180000 poll-ms 2000}}]
  (let [graph (g/graph panel (or config {}))]
    (-> (json-post (str base "/prompt")
                   {:prompt graph :client_id "ghosthacker-produce"})
        (.then
...
(defn base-url
  "Env -> the ComfyUI root, or nil. `aget` rather than js->clj: the latter
  yields a Function under nbb and every lookup comes back nil."
  []
